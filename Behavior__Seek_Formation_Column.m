...
%Event Mouse click
    function cursorPosition = cursorClickCallback(o,e)
        ...
        SaveMousePosition = plot(MousePosition(1), MousePosition(2), 'o','MarkerSize',15,'MarkerFaceColor','yellow','Color','yellow');
        delete(saveText);
        saveText = text(MousePosition(1) + 30, MousePosition(2)+ 10, '');
    end
...
while (timeTick < TimeSteps)
    ...
    force_arr = steer_seek(Blues(1,:), MousePosition); %Move toward the mouse
    avd_force1=steer_collision_avoidance(Blues(1,:));
    force = force_arr + avd_force1*0.5;
    Blues(1,:) = applyForce(Blues(1,:), force);
    
    %% Moving Blues
    Leader = Blues(1, :);            
    [Blues, BluesIndex] = steer_Seek_Formation_Column(MousePosition, Blues, Leader, D_Behind);
