...
if (isOnLeaderSight(CurrentBoid, Leader))
    evade_force = 0.2*steer_evade(CurrentBoid, Leader(1:3), Leader(4:6), FleeDistance);
else
    evade_force = [0,0,0];  
end

avd_force = 1 * steer_collision_avoidance(CurrentBoid); 
force = 0.5 * steer_arrival(CurrentBoid, Target);

% kiem tra gap vat can
    if check_reach_obstacles(CurrentBoid) == 1
        pursue_force = 0;
        avd_force = 0.01 * steer_collision_avoidance(CurrentBoid); 
        force = 0.5 * steer_arrival(CurrentBoid, Target);
    else
        pursue_force = 0.9*steer_pursue_arrival(CurrentBoid, p_following);
    end
